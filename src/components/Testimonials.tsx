  ...
  return (
    <section className="py-28 relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: "radial-gradient(ellipse 60% 50% at 50% 100%, rgba(0,255,65,0.04) 0%, transparent 70%)" }} />
      <div ref={ref} className="max-w-7xl mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.6 }} className="text-center mb-16">
          <p className="text-xs font-semibold tracking-[0.3em] uppercase mb-4" style={{ color: "#00FF41" }}>Kundenstimmen</p>
          <h2 className="font-heading text-[clamp(2.5rem,5vw,4.5rem)] leading-none">
            WAS UNSERE <span style={{ color: "#00FF41" }}>KUNDEN SAGEN</span>
          </h2>
        </motion.div>
          ...
          {testimonials.map((t, i) => (
            <motion.div key={t.name}
              ...
              <div className="flex gap-0.5 mb-5">
                {[...Array(5)].map((_, j) => (
                  <svg key={j} width="14" height="14" viewBox="0 0 24 24" fill="#00FF41">
