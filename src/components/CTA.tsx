    ...
    <section id="kontakt" className="py-28 relative overflow-hidden">
      <div ref={ref} className="max-w-5xl mx-auto px-6">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={inView ? { opacity: 1, scale: 1 } : {}}
          ...
          style={{ background: "linear-gradient(135deg, #001a0a 0%, #000e06 50%, #0A0A0A 100%)", border: "1px solid rgba(0,255,65,0.2)" }}>
          <div className="absolute inset-0 pointer-events-none"
            style={{ background: "radial-gradient(ellipse 80% 60% at 50% -10%, rgba(0,255,65,0.18) 0%, transparent 65%)" }} />
          <motion.div animate={{ opacity: [0.5, 0.8, 0.5] }} transition={{ duration: 4, repeat: Infinity }}
            className="absolute top-0 left-0 right-0 h-px"
            style={{ background: "linear-gradient(90deg, transparent, #00FF41, #39FF14, #00FF41, transparent)" }} />

          <motion.p initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
            transition={{ delay: 0.15 }}
            className="text-xs font-bold tracking-[0.35em] uppercase mb-4 relative" style={{ color: "#00FF41" }}>
            Starte jetzt
          </motion.p>
          <motion.h2 initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
            transition={{ delay: 0.25 }} className="font-heading leading-none mb-5 relative"
            style={{ fontSize: "clamp(2.5rem,6vw,5.5rem)" }}>
            BEREIT FÜR DEINE<br />
            <span style={{ color: "#00FF41" }}>TRANSFORMATION?</span>
          </motion.h2>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
            transition={{ delay: 0.35 }} className="text-white/45 text-lg max-w-lg mx-auto mb-10 relative">
            Buche dein kostenloses Erstgespräch — ohne Verpflichtung, mit echten Ergebnissen.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
            transition={{ delay: 0.45 }} className="flex flex-wrap gap-4 justify-center relative">
            ...
            <a href="tel:+49123456789"
              ...
              style={{ borderColor: "rgba(0,255,65,0.25)", color: "rgba(255,255,255,0.7)" }}>
              📞 Jetzt anrufen
            </a>
          </motion.div>
        </motion.div>
