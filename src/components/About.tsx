  ...
  return (
    <section id="coaching" className="py-32 relative overflow-hidden">
      <div className="absolute right-0 top-0 bottom-0 w-1/2 pointer-events-none"
        style={{ background: "radial-gradient(ellipse 80% 60% at 100% 50%, rgba(0,255,65,0.06) 0%, transparent 70%)" }} />
      <div ref={ref} className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <motion.div initial={{ opacity: 0, x: -60 }} animate={inView ? { opacity: 1, x: 0 } : {}}
            ...
            <div className="aspect-[4/5] rounded-3xl overflow-hidden" style={{ border: "1px solid rgba(0,255,65,0.1)" }}>
              <img src="https://images.unsplash.com/photo-1571019614242-c5c5dee9f50b?w=900&q=80"
                alt="Personal Coach" className="w-full h-full object-cover" />
            </div>
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={inView ? { opacity: 1, scale: 1 } : {}}
              ...
              style={{ background: "linear-gradient(135deg, #00FF41, #39FF14)" }}>
          ...
          <motion.div initial={{ opacity: 0, x: 60 }} animate={inView ? { opacity: 1, x: 0 } : {}}
            ...
            <div className="inline-flex items-center gap-2 mb-6 px-4 py-2 rounded-full text-xs font-medium tracking-widest uppercase"
              style={{ border: "1px solid rgba(0,255,65,0.3)", color: "#00FF41", background: "rgba(0,255,65,0.05)" }}>
              Dein Coach
            </div>
            <h2 className="font-heading text-[clamp(2.5rem,5vw,4.5rem)] leading-none mb-6">
              DEIN COACH.<br />
              <em className="not-italic" style={{ color: "#00FF41" }}>DEIN PARTNER.</em>
            </h2>
            ...
            <ul className="space-y-3 mb-10">
              {qualities.map((q, i) => (
                <motion.li key={q} initial={{ opacity: 0, x: 20 }} animate={inView ? { opacity: 1, x: 0 } : {}}
                  transition={{ duration: 0.5, delay: 0.4 + i * 0.08 }}
                  className="flex items-center gap-3 text-sm text-white/70">
                  <span className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 text-xs"
                    style={{ background: "rgba(0,255,65,0.15)", color: "#00FF41" }}>✓</span>
                  {q}
                </motion.li>
              ))}
            </ul>
